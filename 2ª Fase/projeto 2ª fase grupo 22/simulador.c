...
int main(int argc, char **argv)
{
	...
	configs.prob_cliente_ser_prio			= conf[9];
	...
	time(&tempo);
	meuTempo = localtime(&tempo);
	sprintf(buffer, "%d %d %d 1 1 %d %d\n", meuTempo->tm_hour, meuTempo->tm_min, meuTempo->tm_sec, configs.numero_produtos, configs.stock_inicial_produtos);
	...
	do
	{
		...
		time(&tempo);
		meuTempo = localtime(&tempo);
		sprintf(buffer, "%d %d %d %s", meuTempo->tm_hour, meuTempo->tm_min, meuTempo->tm_sec, input);
		send(sockfd, buffer, sizeof(buffer), 0);
	}while(sair != 1);
